...
def test_gen_insert_table():
    ...
    assert (
        lib.gen_insert_table(
            "test", dict(col_a="INTEGER", col_b="VARCHAR", col_c="FLOAT")
        )
        == expected
    )
...
def test_gen_select():
    ...
    assert (
        lib.gen_select("test", ["SUM(col_a)", "MAX(col_b)", "MIN(col_c)"]) == expected
    )


def test_gen_select_w_groupby():
    expected = (
        "SELECT SUM(col_a), MAX(col_b), MIN(col_c) FROM test GROUP BY col_v, " "col_w;"
    )
    assert (
        lib.gen_select(
            "test", ["SUM(col_a)", "MAX(col_b)", "MIN(col_c)"], ["col_v", "col_w"]
        )
        == expected
    )
